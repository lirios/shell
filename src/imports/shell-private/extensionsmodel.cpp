    ...
    for (const auto &directory : directories) {
        ...
        for (const auto &info : entries) {
            ...
            if (model->m_extensionNames.contains(name)) {
                qCWarning(lcShell, "Skipping \"%s\" status area extension: duplicate name",
                          qPrintable(name));
                continue;
            } else {
            ...
            if (!mainScriptInfo.exists() || !mainScriptInfo.isFile()) {
                qCWarning(lcShell, "Skipping \"%s\" status area extension: path \"%s\" doesn't exist or it's not a file",
