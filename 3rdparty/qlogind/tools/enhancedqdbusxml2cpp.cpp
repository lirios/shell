...
{
    QString name = method.annotations.value(QLatin1String("org.qtproject.QtDBus.MethodName"));
    if (!name.isEmpty())
        return name;
...
static void writeProxy(const QString &filename, const QDBusIntrospection::Interfaces &interfaces)
{
    ...
    hs << "#include <QtCore/QObject>" << endl
       ...
       << "#include \"enhancedqdbusabstractinterface.h\"" << endl;
    ...
    foreach (const QDBusIntrospection::Interface *interface, interfaces) {
        ...
        hs << "#include \"qlogind_export.h\"" << endl;
        hs << "class QLOGIND_EXPORT " << className << ": public EnhancedQDBusAbstractInterface" << endl
           << "{" << endl
           << "    Q_OBJECT" << endl;
        ...
        cs << className << "::" << className << "(const QString &service, const QString &path, const QDBusConnection &connection, QObject *parent)" << endl
           << "    : EnhancedQDBusAbstractInterface(service, path, staticInterfaceName(), connection, parent)" << endl
           << "{" << endl
           << "}" << endl
           << endl
        ...
        foreach (const QDBusIntrospection::Property &property, interface->properties) {
            ...
            if (property.annotations.value("org.freedesktop.DBus.Property.EmitsChangedSignal", "true") == QLatin1String("true"))
                hs << " NOTIFY " << getter + "Changed";
        ...
        hs << "//property changed signals" << endl << endl;
        foreach (const QDBusIntrospection::Property &property, interface->properties) {
            if (property.annotations.value("org.freedesktop.DBus.Property.EmitsChangedSignal", "true") == QLatin1String("true")) {
                hs << "    ";
                hs << "void " << propertyGetter(property) << "Changed();" << endl;
